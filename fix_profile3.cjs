...
const regex = /<\/div>\s*<\/div>\s*<\/div>\s*<div className="mt-6 pt-6 border-t border-black\/10 dark:border-white\/10">[\s\S]*?<\/div><\/div><\/div><\/div><\/div><BirthdaySettings \/>/m;
...
profile = profile.replace(regex, replacement);
